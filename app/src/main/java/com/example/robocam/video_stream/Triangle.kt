...
class Triangle(context: Context) {
    ...
    private var iconVertexBuffer: Int = 0
    private var iconTexCoordBuffer: Int = 0

    // Scale for icon size (adjust these values for desired icon dimensions)
    private val iconWidthScale = 0.5f  // 30% of screen width
    private val iconHeightScale = 0.5f // 30% of screen height
    ...
    private val iconVerticesCoords = floatArrayOf(
    ...
    private val iconTextureCoords = floatArrayOf(
        0.0f, 0.0f,  // Top-left
        1.0f, 0.0f,  // Top-right
    ...
    init {
        ...
        //iconTextureID = setupTexture(context, "models/mind.png")
        iconVertexBuffer = setupBuffer(iconVerticesCoords)
        iconTexCoordBuffer = setupBuffer(iconTextureCoords)
    }
    fun setupDialogTexture(bitmap: Bitmap){
        iconTextureID = setupTextureDialog(bitmap)
    }
    ...
    private fun drawIcon(){
        ...
        GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, iconVertexBuffer)
        GLES20.glEnableVertexAttribArray(positionAttrib)
        GLES20.glVertexAttribPointer(positionAttrib, 3, GLES20.GL_FLOAT, false, 0, 0)

        GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, iconTexCoordBuffer)
        GLES20.glEnableVertexAttribArray(texCoordAttrib)
        GLES20.glVertexAttribPointer(texCoordAttrib, 2, GLES20.GL_FLOAT, false, 0, 0)
